...
APE.Client.prototype.load = function(config){
	...
	if(location.href.indexOf('https') == 0) {
		config.secure = true; //config.sercure || APE.Config.secure;
	}
...
APE.Config.baseUrl = protocol + document.domain + (document.location.port ? ':' + document.location.port : '')+ '/js/lib/ape-jsf/'; //APE JSF 
APE.Config.domain = 'auto'; 
APE.Config.server = document.domain + ':' + port;// + ((document.location.port % 2) ? '6969' : '6968'); //APE server URL
